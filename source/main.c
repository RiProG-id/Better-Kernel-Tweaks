...
#define LOG_PATH "/storage/emulated/0/Android/BKT.log"
...
int is_gpu_device(const char *name) {
    return strcmp(name, "kgsl-3d0") == 0 ||
           strstr(name, ".mali") ||
           strstr(name, ".gpu");
}

int find_gpu_path_recursive(const char *search_path, char *gpu_path, size_t path_buffer_size) {
...
int main() {
    if (getuid() != 0) {
        printf("ERROR: This program requires ROOT access!\n");
        return 1;
    }

    remove(LOG_PATH);
    log_msg("[*] Tweaks started...");
    ...
    for (int cpu = 0; cpu < 16; cpu++) {
        ...
        snprintf(path_buffer, sizeof(path_buffer),
                 "%s/scaling_available_governors", cpu_path);
    ...
    for (int type = 0; type < 2; type++) {
        int limit = (type == 0) ? 6 : 2;

        for (int i = 0; i < limit; i++) {
            char block_path[256];
            char path_buffer[256];
            char schedulers_list[256];

            if (type == 0)
                snprintf(block_path, sizeof(block_path),
                         "/sys/block/sd%c/queue", 'a' + i);
            else
                snprintf(block_path, sizeof(block_path),
                         "/sys/block/mmcblk%d/queue", i);

            if (!is_directory(block_path)) continue;

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/scheduler", block_path);
            ...
            if (f) {
                ...
                const char *sched =
                    strstr(schedulers_list, "bfq") ? "bfq" :
                    strstr(schedulers_list, "mq-deadline") ? "mq-deadline" :
                    "none";
            ...
            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/read_ahead_kb", block_path);
            write_file(path_buffer, "64");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/nr_requests", block_path);
            write_file(path_buffer, "256");
        }
    }

    log_msg("[+] I/O schedulers applied.");
    ...
    if (f) {
        ...
        const char *active_congestion =
            strstr(congestion_list, "bbr") ? "bbr" :
            strstr(congestion_list, "cubic") ? "cubic" :
            strstr(congestion_list, "westwood") ? "westwood" :
            "reno";
    ...
    if (get_gpu_path(gpu_device_path, sizeof(gpu_device_path))) {
        ...
        if (is_adreno) {
            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/throttling", gpu_device_path);
            write_file(path_buffer, "0");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/force_clk_on", gpu_device_path);
            write_file(path_buffer, "1");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/force_rail_on", gpu_device_path);
            write_file(path_buffer, "1");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/force_no_nap", gpu_device_path);
            write_file(path_buffer, "1");
        } else {
            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/dvfs", gpu_device_path);
            write_file(path_buffer, "0");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%s/power_policy", gpu_device_path);
        ...
        if (gpu_frequency_path) {
            snprintf(path_buffer, sizeof(path_buffer),
                     "%saging_mode", gpu_frequency_path);
            write_file(path_buffer, "disable");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%slimit_table", gpu_frequency_path);
            write_file(path_buffer, "0 0 0");

            snprintf(path_buffer, sizeof(path_buffer),
                     "%sgpm_mode", gpu_frequency_path);
            write_file(path_buffer, "1");
        }
